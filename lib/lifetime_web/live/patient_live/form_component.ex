defmodule LifetimeWeb.PatientLive.FormComponent do
  ...
  def update(%{patient: patient} = assigns, socket) do
    changeset = Medical.change_patient(patient)
  ...
  def handle_event("validate", %{"patient" => patient_params}, socket) do
    changeset =
      ...
      |> Medical.change_patient(patient_params)
  ...
  defp save_patient(socket, :edit, patient_params) do
    case Medical.update_patient(socket.assigns.patient, patient_params) do
      {:ok, _patient} ->
        {:noreply,
         socket
  ...
  defp save_patient(socket, :new, patient_params) do
    case Medical.create_patient(patient_params) do
      {:ok, _patient} ->
        {:noreply,
         socket
